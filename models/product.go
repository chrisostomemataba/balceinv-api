...
// Pointer types (*float64, *string, *int) represent nullable columns —
// this is how Go handles NULL in the database. If the pointer is nil, the column is NULL.
type Product struct {
	...
	// Wholesale — optional fields use pointers so they can be NULL
	...
	// Unit tracking
	Unit           string     `gorm:"default:pcs"              json:"unit"` // pcs, btl, crt, kg, L
	PiecesPerUnit  int        `gorm:"column:pieces_per_unit;default:1" json:"pieces_per_unit"`

	CreatedAt      time.Time  `gorm:"autoCreateTime"           json:"created_at"`
	UpdatedAt      time.Time  `gorm:"autoUpdateTime"           json:"updated_at"`

	// Associations — loaded only when you explicitly Preload() them
...
// A product can have multiple barcodes (e.g. single unit vs crate barcode).
type Barcode struct {
	ID        uint      `gorm:"primaryKey;autoIncrement" json:"id"`
...
// PriceHistory maps to the `price_history` table.
// Every time a product price changes, a record is inserted here automatically.
...
// StockAlert maps to the `stock_alerts` table.
// Created automatically when stock drops below the product's minStock threshold.
type StockAlert struct {
	...
	AlertType       string    `gorm:"column:alert_type;not null" json:"alert_type"` // low | out
